...
class UsuarioController {
  ...
  async updateImagem(req, res) {
    ...
    const usuario = await UsuarioModel.findById(id); // Interessante utilizar o .exec() depois de qualquer find
    ...
    const file = req.file;
    const { key } = await uploadFile({ file, ACL: "public-read" }); // Consultor PO sobre acesso dos arquivos
