...
const ConfereAdmin = async (req, res, next) => {
    ...
    try{
        ...
        console.error("Erro ao verificar a permissão do usuario.");
        return res.status(500).json({ message: "Erro ao verificar a permissão do usuario." });
    }
}

const destroy = validateRequest({
    params: z.object({
        id: z.custom(mongoose.isValidObjectId, "o id não é válido para esta operação."),
    }),
});
